<template>
  <div id="rightArticle" @click="gotoArticle()">
    <span>{{ index+1 }}</span>&nbsp;&nbsp;<div class="title">{{ article.title }}</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getArticle(){
      ...
      const {data: res} = await this.$http.post('/discussPost/getDiscussPostCommended',this.qs.stringify(data));
