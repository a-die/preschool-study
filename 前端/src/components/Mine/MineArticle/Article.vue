<template>
  <div class="ourArticle" @click="gotoBlog()">
    <h3>{{article.title}}</h3>
    <div class="ArticleContent">{{ article.title }}</div>
    <div>
      <div class="articleDo">
        <div class="articleInfo">
          <slot name="articleInfo">
            <span>分类:&nbsp;&nbsp;{{ article.btype }}</span>
            <span><i class="far fa-eye"></i>&nbsp;&nbsp;{{ article.comment_count }}</span>
        ...
        <div v-if="isMe">
          <slot name="articleDo">
            <button @click.stop="goWrite()">编辑</button>
            <button @click.stop="deleteArticle()">删除</button>
          </slot>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    time(){
      let t;
      if(this.article.update_time){
        t = new Date(this.article.update_time);
      }else{
        t=new Date();
      }
  ...
  methods: {
    gotoBlog(){
      ...
      if(this.colectted) this.$router.push({name: 'blog', params:{id: this.article.discussPostId}});
      else this.$router.push({name: 'blog', params:{id: this.article.id}});
    },
    // 删除
    async deleteArticle(){
      // /user/information
...
.ourArticle:hover{
  background: var(--background);
  border: 4px solid var(--very-light-blue);
  box-shadow: 0 2px 4px 0 rgba(0, 0, 0, 0.08);
}
</style>