    ...
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                ...
                <el-table-column label="状态" align="center" width="100">
                    <template #default="scope">
                        <el-tag :type="
                                scope.row.status === 0? 'success': 'primary'
                            " @click="upadteStatus(scope.row)">{{ scope.row.status ==0?'正常':'冻结' }}</el-tag>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="30%">
            <el-form label-width="70px">
				<el-form-item label="用户名" v-show="false">
...
export default {
    ...
    setup() {
        ...
        const getData = () => {
			...
			try {
				//查找所有的用户数量
				axios.get('http://localhost:8081/user/getUserNum'
				)
				.then(res=> {
				...
				//查找当前页面的用户
				axios.post('http://localhost:8081/user/showAllByPage',
					qs.stringify({
		...
		const getDataByname = () =>  {
			...
			try {
				...
				if(query.name=="")getData();
					...
					axios.post('http://localhost:8081/user/getUserListByName',
						qs.stringify({
						username : query.name
						})
					)
